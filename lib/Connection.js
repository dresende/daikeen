...
class Connection {
	...
	update() {
		this.getSensorInformation((err) => {
			if (err) debug(err);

			this.getControlInformation((err) => {
				if (err) debug(err);
	...
	set(path, data, next) {
		...
		http.get(`http://${this.#address}${path}?${qs.stringify(request)}`, (res) => {
			let buffer = "";
	...
	get(path, next) {
		http.get(`http://${this.#address}${path}`, (res) => {
			let buffer = "";
